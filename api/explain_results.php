...
if ($calculator_type === 'vanguard-pas-vs-target-date') {
    $system_prompt .= " This calculator isolates costs with the same gross return for PAS and all three self-managed buckets. Distinguish PAS advisory fee assumptions and dollars from PAS underlying fund expense assumptions and dollars. Their sum is the modeled PAS all-in cost, not a universal Vanguard rate. The underlying expense default is generic, not a verified client expense. Lead with PAS total modeled costs, self-managed fund expenses (no advisory fee), and their signed direct difference. Separately explain the ending portfolio difference; it is not all fees and is not an additional charge. The residual is compounding and, when actual withdrawals differ, withdrawal effects. Dollar scenarios apply the same inflation-adjusted spending requirement to both alternatives; identify any unfunded spending. Legacy percentage scenarios can have different withdrawals. Annual timing is growth, fees once on grown assets, then spending. Withdraw Conservative, then Moderate, then Aggressive without replenishment; sequencing alone does not create extra performance. Keep the explanation to deterministic costs and spending, not stochastic probabilities or investment predictions.";
}
