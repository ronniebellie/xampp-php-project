...
// Chart Section (requires GD or Imagick for PNG with alpha; otherwise skip chart so PDF still generates)
$canEmbedPng = extension_loaded('gd') || extension_loaded('imagick');
if (isset($data['chartImage']) && !empty($data['chartImage']) && $canEmbedPng) {
    $pdf->SetFont('helvetica', 'B', 16);
    $pdf->SetTextColor(102, 126, 234);
    ...
    unlink($tempFile);
    $pdf->Ln(85);
} elseif (isset($data['chartImage']) && !empty($data['chartImage']) && !$canEmbedPng) {
    $pdf->SetFont('helvetica', 'B', 16);
    $pdf->SetTextColor(102, 126, 234);
    $pdf->Cell(0, 8, 'Account Balance & RMD Over Time', 0, 1);
    $pdf->SetTextColor(0, 0, 0);
    ...
    $pdf->Cell(0, 6, '(Chart omitted — server needs PHP GD or Imagick extension for images.)', 0, 1);
    $pdf->Ln(8);
}
...
header('Content-Disposition: attachment; filename="RMD_Analysis_' . date('Y-m-d') . '.pdf"');
