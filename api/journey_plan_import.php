<?php
/**
 * POST /api/journey_plan_import.php
 * First-time browser → account import. Refuses to overwrite an existing cloud plan.
 */
...
if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    journey_plan_json_response([
        ...
        'message' => 'Use POST to import a Journey plan.',
    ], 405);
}
...
if ($userId <= 0) {
    journey_plan_json_response([
        ...
        'message' => 'Sign in to save this browser’s Journey to your account.',
...
$result = journey_plan_import($conn, $userId, $payload, $clientUpdatedAt);
if (empty($result['ok'])) {
    $error = (string) ($result['error'] ?? 'import_failed');
    $status = 400;
    if ($error === 'premium_required' || $error === 'invalid_csrf') {
        $status = 403;
    } elseif ($error === 'already_exists' || $error === 'conflict') {
        $status = 409;
...
journey_plan_json_response([
    ...
    'plan' => $result['plan'],
    'csrfToken' => rb_csrf_token(),
    'message' => 'Saved this browser’s Journey to your account.',
], 200);
