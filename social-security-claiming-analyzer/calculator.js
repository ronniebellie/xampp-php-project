...
function saveScenario() {
    ...
    rbScenarioFetch(SS_API_BASE + '/api/save_scenario.php', {
        method: 'POST',
        ...
        if (choice.toLowerCase().startsWith('d')) {
            ...
            if (index >= 0 && index < data.scenarios.length) {
                ...
                if (confirm(`Delete "${scenarioDisplayName(scenario)}"? This cannot be undone.`)) {
                    rbScenarioFetch(SS_API_BASE + '/api/delete_scenario.php', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
...
function explainResults() {
    ...
    .then(text => {
        ...
        showExplainModal(data.explanation, { calculatorType: 'social-security-claiming-analyzer', resultsSummary: summary });
    })
    .catch(err => {
        if (btn) { btn.disabled = false; btn.textContent = origText; }
